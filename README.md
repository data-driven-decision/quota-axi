...
- A transport-class failure such as this one (never a definitive `auth_required`) may still report a formerly fresh snapshot instead of a bare error, explicitly marked `stale: true` with the failing `error` attached and aged out per-window against each window's own `resetsAt`/`windowSeconds`; see [Cache](#cache).
