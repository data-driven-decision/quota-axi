...
- This particular failure is a plain HTTP 404, reported as a bare error rather than a stale-cache reuse: only timeouts, rate limits, and 5xx responses are stale-eligible, so a WAF block never masks itself behind an old-looking snapshot. Other transport-class failures (never a definitive `auth_required`) may still report a formerly fresh snapshot instead, explicitly marked `stale: true` with the failing `error` attached and aged out per-window against each window's own `resetsAt`/`windowSeconds`; see [Cache](#cache).
...
| MiniMax cache aging                    | A transport, timeout, rate-limit, or provider failure (never a definitive `auth_required`, which retires the cache instead) may reuse a formerly fresh same-provider snapshot, marked `stale: true` with the failing `error` attached. Each window is aged out independently against its own `resetsAt`, or its own `windowSeconds` when no `resetsAt` is present; a window with neither is dropped rather than republished indefinitely, since its duration cannot be verified. |
